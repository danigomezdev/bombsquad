...
class PartyWindow(bui.Window):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._edit_movements_button = bui.buttonwidget(
            ...
            autoselect=True,
            button_type='square',
            on_activate_call=bs.CallPartial(doTestButton, self),
            color=self.bg_color,
            iconscale=1.2)
...
def new_bomb_init(func):
    def setting(*args, **kwargs):
        ...
        if babase.app.config.get(
            ...
            color = (1.0, 0.5, 0.0)
            bs.timer(1, bs.CallPartial(count_bomb, *args, count='2', color=color))
            color = (1.0, 0.15, 0.15)
            bs.timer(2, bs.CallPartial(count_bomb, *args, count='1', color=color))
...
def _init_spaz_(func):
    def wrapper(*args, **kwargs):
        ...
        bs.timer(1.001, bs.CallPartial(invincible))
...
class NewBotSet(SpazBotSet):
    ...
    def start_moving(self) -> None:
        ...
        self._bot_update_timer = bui.AppTimer(
            0.05, bs.WeakCallPartial(self._update), repeat=True
        )
...
class PracticeWindow(bui.Window):
    ...
    def __init__(self,
        ...
        self._restore_state()
        self._tab_row.update_appearance = self.custom_update

        # Force initial colors
        if self._current_tab is not None:
            self._tab_row.update_appearance(self._current_tab)
        else:
            # If there is no tab selected yet, we force the first one
            first_tab = list(self._tab_row.tabs.keys())[0]
            self._tab_row.update_appearance(first_tab)
    ...
    def custom_update(self, tab_id_selected):
        """Updates the tab colors according to the selected tab."""
        for tid, tab in self._tab_row.tabs.items():
            if tid == tab_id_selected:
                # Selected tab
                bui.buttonwidget(
                    edit=tab.button,
                    color=(0.6, 0.15, 0.15),      
                    textcolor=(1.0, 0.9, 0.9),    
                )
            else:
                # Tab UnSelected
                bui.buttonwidget(
        ...
        for y in range(rows):
            for x in range(columns):
                ...
                btn = bui.buttonwidget(
                    ...
                    tint_color=tint1,
                    tint2_color=tint2,
                    on_activate_call=bui.CallStrict(self._select_character,
        ...
        for y in range(rows):
            for x in range(columns):
                ...
                btn = bui.buttonwidget(
                    ...
                    label='',
                    color=(1, 1, 1),
                    on_activate_call=bui.CallStrict(self._select_power,
...
VERSION = "3.1"
UPDATE_URL = "https://raw.githubusercontent.com/danigomezdev/bombsquad/refs/heads/main/PracticeTools/PracticeTools.json"
...
class UpdateWindow(bui.Window):
    def __init__(s, start_check=True):
        ...
        if not lpm_manages:
            s._auto_key = 'AutoUpdate_PracticeTools'
    ...
    def _rc(s):
        ...
        if info.get("update_available") and r:
            if s._auto_update:
                ...
                bui.screenmessage("Auto-updated: PracticeTools", color=(0,1,0))
            else:
                s._info = info
    ...
    def _rd(s, info):
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=60) as resp: c = resp.read()
            with open(os.path.join(_E["python_directory_user"], "PracticeTools.py"), "wb") as f: f.write(c)
