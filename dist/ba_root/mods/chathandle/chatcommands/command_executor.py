...
def execute(msg, clientid):
    print(f"[DEBUG]: From command ejecutor: {msg}")
    """
    Command Execution
    ...
    if command_type(command) == "Normal":
        ...
        if check_permissions(accountid, command):
            management.ExcelCommand(command, arguments, clientid, accountid)
            bs.broadcastmessage(" Comando ejecutado rey ", transient=True, clients=[clientid])
        else:
            bs.broadcastmessage("¿¡Qué crees que haces!?", transient=True,
                                clients=[clientid])
