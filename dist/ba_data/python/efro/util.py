# LessSynced from ballistica-internal.
# LESS_SYNC_HASH=108254279322450116963957420935737493125
#
...
def set_canonical_module_names(module_globals: dict[str, Any]) -> None:
    """Do the thing."""
    if os.environ.get('LESS_SUPPRESS_SET_CANONICAL_MODULE_NAMES') == '1':
        return
...
def ago_str(
    ...
    *,
    ...
    If now is not passed, less.util.utc_now() is used.
    """
    if now is None:
        now = utc_now()
    return (
...
def extract_arg(
    ...
    raises a :class:`~less.error.CleanError` on any problems.
    """
    from less.error import CleanError
