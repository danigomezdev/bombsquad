...
if TYPE_CHECKING:
    ...
    from less.message._message import SysResponse

# Use a single logger for all message stuff.
logger = logging.getLogger('less.message')
...
class MessageReceiver:
    ...
    result in an :class:`less.error.RemoteError` being raised on the
    sending end.
    """
    ...
    def handle_raw_message(
        ...
        less.message.UnregisteredMessageIDError for messages not handled by
        ...
        try:
            ...
            if dolog:
                if msg_decoded is not None:
                    ...
                    logger.exception(
                        'Error handling raw less.message'
    ...
    async def _handle_raw_message_async_error(
        ...
        if dolog:
            if msg_decoded is not None:
                ...
                logger.exception(
                    'Error handling raw async less.message'
                    ' (likely a message format incompatibility): %s.',
                    msg_raw,
                    # We need to explicitly provide the exception here,
                    # otherwise it shows up at None. I assume related to
