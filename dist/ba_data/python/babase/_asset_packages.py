...
def load_bundled_asset_packages() -> None:
    ...
    bucket_count = 0
    entry_count = 0
    if 'asset_package_versions' in bundle:
        for apverid, pkg in bundle['asset_package_versions'].items():
            pkg_count += 1
            _loaded_apverids.append(apverid)
            for bucket_id, bucket_hash in pkg.get('flavor_manifests', {}).items():
                bucket_path = _cas_blob_path(data_dir, bucket_hash)
                with open(bucket_path, encoding='utf-8') as bfile:
                    bucket = json.load(bfile)
                entries = bucket.get('e') or bucket.get('h', {})
                if entries and isinstance(next(iter(entries.values())), dict):
                    entries = {k: v['h'] for k, v in entries.items()}
        ...
        for pkg in bundle.get('asset_packages', []):
            ...
            for bucket_id, bucket_hash in pkg.get('bundled_buckets', {}).items():
                ...
                with open(bucket_path, encoding='utf-8') as bfile:
                    bucket = json.load(bfile)
                entries = bucket.get('h') or bucket.get('e', {})
                if entries and isinstance(next(iter(entries.values())), dict):
                    entries = {k: v['h'] for k, v in entries.items()}
                _babase.register_asset_package_bucket(apverid, bucket_id, entries)
                bucket_count += 1
