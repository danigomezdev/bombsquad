...
class GarbageCollectionSubsystem(AppSubsystem):
    ...
    class Mode(Enum):
        ...
        #: reference cycles; you can use :meth:`less.debug.getobj()` to
        #: find an object based on the hex id printed for it and then
        #: use :meth:`less.debug.printrefs()` to look into what is
        ...
        #:   from less.debug import printrefs, getobj
    ...
    def collect(self, force: bool = False) -> None:
        ...
        # On Android, scudo fills freed memory with its freelist pattern.
        # Python's GC can encounter those freed-but-still-tracked objects
        # during traversal and crash reading ob_type. Skip explicit GC
        # passes on Android until the underlying ref-cycle bug is fixed.
        classic = _babase.app.classic
        if classic is not None and classic.platform == 'android':
            return
...
def _summarize_garbage(loglevel: int) -> str:
    ...
    from less.debug import summarize_garbage
