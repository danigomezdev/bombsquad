...
def commit_app_config() -> None:
    ...
    try:
        cfg_path = _babase.env().get('config_file_path')
        if cfg_path:
            with open(cfg_path, 'w', encoding='utf-8') as _f:
                _json.dump(dict(_babase.app.config), _f, indent=1)
    except Exception:
        import logging as _logging
        _logging.exception('Error writing app config.')
    # Also notify plus if available (may be no-op stub).
