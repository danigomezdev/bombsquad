...
import urllib3
from less.logging import LogLevel

if TYPE_CHECKING:
    from typing import Any

    from less.logging import LogEntry, LogHandler
...
def on_main_thread_start_app() -> None:
    ...
    if not TYPE_CHECKING:
        ...
        # use site.py.
        # Android monolithic: __main__.__builtins__ may not be accessible.
        try:
...
def _bootstrap_networking() -> None:
    ...
    from less.util import strict_partial
    ...
    # less.util.strip_exception_tracebacks() after handling them.
...
def _warm_start_bootstrap_connection(pool: urllib3.PoolManager) -> None:
    from less.error import is_urllib3_communication_error
    from less.util import strip_exception_tracebacks
    from babase._logging import netlog
    import _babase

    url = 'https://api.bombsquad.lat/ping'
...
def _do_pycache_upkeep() -> None:
    ...
    from less.util import prune_empty_dirs
