...
from less.terminal import Clr
from less.error import CleanError
...
def _resolve_api_key(project_root: Path) -> str | None:
    ...
    2. ``ballistica_api_key`` in the localconfig.json pointed to by
       ``LESS_LOCALCONFIG_PATH`` (absolute or relative to
       ``project_root``). This matches the override used by
       ``lesstools.project.getlocalconfig`` and is how CI runs on
    ...
    cfgpaths: list[Path] = []
    override = os.environ.get('LESS_LOCALCONFIG_PATH')
    if override:
        opath = Path(override)
...
class App:
    ...
    def run(self) -> int:
        ...
        # module via lesssync — historically this was tools/batools
    ...
    def _handle_downloads(self, downloads: ResponseData.Downloads) -> None:
        from less.util import data_size_str
