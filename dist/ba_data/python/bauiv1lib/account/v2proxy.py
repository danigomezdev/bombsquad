...
from less.error import CommunicationError
...
import os as _os
try:
    _V2PROXY_LOGPATH = f'/tmp/v2proxy_{_os.getpid()}.log'
    _V2PROXY_LOG = open(_V2PROXY_LOGPATH, 'w')
    _V2PROXY_OK = True
except Exception:
    _V2PROXY_OK = False

def _v2log(msg: str) -> None:
    if not _V2PROXY_OK:
        return
    try:
        _V2PROXY_LOG.write(f'{time.monotonic():.3f} {msg}\n')
        _V2PROXY_LOG.flush()
    except Exception:
        pass
...
class V2ProxySignInWindow(bui.Window):
    ...
    def _update(self) -> None:
        ...
        if (
            ...
            _v2log(f'waiting for cloud. connected={plus.cloud.connected}')
            return

        _v2log('sending LoginProxyRequestMessage')
    ...
    def _on_proxy_request_response(
        ...
        if isinstance(response, Exception):
            _v2log(f'proxy response error: {type(response).__name__}: {response}')
            self._set_error_state(f'response exc ({type(response).__name__})')
            self._complete = True
            return

        _v2log(f'proxy response OK, url={response.url[:50]}...')
        self._complete = True
    ...
    def _show_standard_sign_in_ui(
        ...
        _v2log(f'standard signin UI. url len={len(address)}')
        ...
        qr_size = 270
        _v2log(f'generating QR for url len={len(address)}')
        try:
            qr_tex = bui.get_qrcode_texture(address)
            _v2log('QR texture created OK')
        except Exception as exc:
            _v2log(f'QR texture FAILED: {exc}')
            qr_tex = None
        if qr_tex is not None:
            bui.imagewidget(
                parent=self._root_widget,
                position=(
