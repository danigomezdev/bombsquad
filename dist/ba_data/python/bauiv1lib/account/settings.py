...
class AccountSettingsWindow(bui.MainWindow):
    ...
    def __init__(
        ...
        self._sign_in_v3_button: bui.Widget | None = None
        ...
        self._v3_signed_in = bool(bui.app.config.get('V3 Token', ''))
        self._v3_signed_in = bool(bui.app.config.get('V3 Token', ''))
        ...
        # Always want to show our web-based v3 login option.
        self._show_sign_in_buttons.append('V3')
    ...
    def _update(self) -> None:
        ...
        v3_now = bool(bui.app.config.get('V3 Token', ''))
        if v3_now != self._v3_signed_in:
            self._v3_signed_in = v3_now
    ...
    def _refresh(self) -> None:
        ...
        show_signed_in_as = self._v1_signed_in or self._v3_signed_in
        ...
        show_sign_in_benefits = not self._v1_signed_in and not self._v3_signed_in
        ...
        show_v3_sign_in_button = (
            v1_state == 'signed_out'
            and not sign_in_in_progress
            and 'V3' in self._show_sign_in_buttons
            and not self._v3_signed_in
        )
        ...
        show_create_account_button = show_v3_sign_in_button
        ...
        if self._v3_signed_in:
            self._sub_height += 130
        ...
        self._account_name_text: bui.Widget | None = None
        if show_signed_in_as:
        ...
        if self._v3_signed_in:
            ...
            bui.buttonwidget(
                parent=self._subcontainer,
                position=((self._sub_width - btn_w) * 0.5, v + 60),
                size=(btn_w, btn_h),
                label='Manage Account',
                icon=bui.gettexture('settingsIcon'),
                iconscale=1.2,
                color=(0.4, 0.45, 0.55),
                on_activate_call=self._v3_manage_account,
            )

            bui.buttonwidget(
                parent=self._subcontainer,
                position=((self._sub_width - btn_w) * 0.5, v - 15),
                size=(btn_w, btn_h),
                label='Sign Out',
                color=(0.5, 0.2, 0.2),
                on_activate_call=self._v3_sign_out,
            )
        ...
        if show_v3_sign_in_button:
            button_width = 350
            v -= sign_in_button_space
            self._sign_in_v3_button = btn = bui.buttonwidget(
                parent=self._subcontainer,
                id=f'{self.main_window_id_prefix}|signinv3',
                position=((self._sub_width - button_width) * 0.5, v - 20),
                autoselect=True,
                size=(button_width, 60),
                label='',
                on_activate_call=self._v3_sign_in_press,
            )
            bui.textwidget(
                parent=self._subcontainer,
                draw_controller=btn,
                ...
                text='Sign in with Bombsquad V3',
        ...
        self._achievements_text: bui.Widget | None = None
        if show_achievements_text:
        ...
        self._campaign_progress_text: bui.Widget | None = None
        if show_campaign_progress:
        ...
        self._tickets_text: bui.Widget | None = None
        if show_tickets:
            v -= tickets_space * 0.5
    ...
    def _on_create_account_press(self) -> None:
        from bauiv1lib.account.v3login import V3RegisterWindow
        V3RegisterWindow()
    ...
    def _refresh_account_name_text(self) -> None:
        ...
        try:
            ...
            if v3_user:
                name_str = bui.charstr(bui.SpecialChar.V3_LOGO) + v3_user
            else:
    ...
    def _v3_sign_in_press(self) -> None:
        self._recreate_suppress = bui.MainWindowAutoRecreateSuppress()
        bui.pushcall(self._v3_sign_in)

    def _v3_sign_in(self) -> None:
        from bauiv1lib.account.v3login import V3SignInWindow
        V3SignInWindow(origin_widget=self._sign_in_v3_button)

    def _v3_manage_account(self) -> None:
        from bauiv1lib.account.v3account import V3AccountWindow
        self.main_window_replace(
            lambda: V3AccountWindow(origin_widget=self._sign_in_v3_button)
        )

    def _v3_sign_out(self) -> None:
        import _babase
        cfg = bui.app.config
        cfg['V3 Token'] = ''
        cfg['V3 Username'] = ''
