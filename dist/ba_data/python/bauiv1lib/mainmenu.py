...
class MainMenuWindow(bui.MainWindow):
    ...
    def _refresh(self) -> None:
        ...
        self._restore_v3_session()

    def _restore_v3_session(self) -> None:
        token = bui.app.config.get('V3 Token', '')
        if not token:
            return
        import _babase
        username = bui.app.config.get('V3 Username', '')
        if username:
            _babase.set_account_sign_in_state(True, username)
        from bauiv1lib.account.v3login import V3AccountFetcher
