...
class PlayOptionsWindow(PopupWindow):
    ...
    def _run_selected_playlist(self) -> None:
        ...
        # Clear all UI immediately. Screen is already black at this point so
        # there's no visual artifact, and this ensures IsMainUIVisible()
        # returns false when the new session starts (so touch input works).
        bui.app.ui_v1.clear_main_window(transition='instant')
        if self.root_widget:
