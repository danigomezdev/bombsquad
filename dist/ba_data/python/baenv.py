...
if TYPE_CHECKING:
    ...
    from less.logging import LogHandler
...
class _EnvGlobals:
    ...
    def get(cls) -> _EnvGlobals:
        ...
        name = '_baenv_globals'
        # Android monolithic: setattr on __main__ does not persist reliably.
        # Use sys.modules dict as primary store (dict.__setitem__ works).
        envglobals: _EnvGlobals | None = sys.modules.get(
            '__ballistica_baenv__'
        )
        if envglobals is None:
            envglobals = getattr(__main__, name, None)
        if envglobals is None:
            envglobals = _EnvGlobals()
            sys.modules['__ballistica_baenv__'] = envglobals
            try:
...
def configure(
    *,
    ...
    # versions of less.logging in a single engine run).
    (
        user_python_dir,
        app_python_dir,
        site_python_dir,
    ...
    # that as much stuff as possible (less.logging), etc.) will get its
...
def _create_log_handler(
    ...
    from less.logging import setup_logging, LogLevel
