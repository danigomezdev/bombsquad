...
VERSION = "2.1.5"
UPDATE_URL = "https://raw.githubusercontent.com/danigomezdev/bombsquad/refs/heads/main/CharacterMaker/CharacterMaker.json"
...
class UpdateWindow(bui.Window):
    def __init__(s, start_check=True):
        ...
        if not lpm_manages:
            s._auto_key = 'AutoUpdate_CharacterMaker'
    ...
    def _rc(s):
        ...
        if info.get("update_available") and r:
            if s._auto_update:
                ...
                bui.screenmessage("Auto-updated: CharacterMaker", color=(0,1,0))
            else:
                s._info = info
    ...
    def _rd(s, info):
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=60) as resp: c = resp.read()
            with open(os.path.join(_E["python_directory_user"], "CharacterMaker.py"), "wb") as f: f.write(c)
